...
class StateManagementAgent(BaseAgent):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "  - Use defineStore with setup syntax (Composition API pattern).\n"
            "  - Export composable hooks: export const useAuthStore = defineStore('auth', () => { ... }).\n"
            "  - Use ref() and computed() inside setup stores, NOT the Options API.\n"
            "  - Keep Pinia for CLIENT STATE only (auth, UI, preferences) — NOT for API data fetching.\n"
            "  - Create barrel src/stores/index.ts that re-exports all stores.\n\n"
