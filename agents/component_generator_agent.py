...
class ComponentGeneratorAgent(BaseAgent):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "VUE 3 — CRITICAL RULES (when framework is Vue):\n"
            "- Generate Single File Components (.vue) with <script setup lang=\"ts\">,\n"
            "  <template>, and optionally <style scoped>.\n"
            "- Use Composition API exclusively (ref, reactive, computed, watch, onMounted).\n"
            "- Use defineProps<T>() and defineEmits<T>() for component interfaces.\n"
            "- Import child components — they are auto-registered in <script setup>.\n"
            "- Use Pinia stores via composable: const store = useMyStore().\n"
            "- For data fetching, create composables in src/composables/ (NOT Pinia).\n"
            "- Router: use useRouter() and useRoute() from 'vue-router'.\n"
            "- Do NOT add 'use client' directives — that is Next.js only.\n\n"

            "IMPORT RULES:\n"
            "- Use RELATIVE imports (e.g. '../ui/Button.vue') for importing from other\n"
            "  components within the same src/ tree. Do NOT use @/ path aliases.\n"
            "- For Vue components, always include the .vue extension in imports.\n"
